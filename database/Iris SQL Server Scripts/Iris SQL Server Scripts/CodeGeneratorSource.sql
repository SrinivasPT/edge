...
declare @Result nvarchar(max) = 
N'CREATE OR ALTER FUNCTION ' + codegen.QNAME( @ProcSchemaName) + '.' + codegen.QNAME('Entity_' + @TableName + '_Get') + '(@id INT, @context NVARCHAR(MAX))
RETURNS NVARCHAR(MAX)
AS BEGIN
...
RETURNS NVARCHAR(MAX)
AS BEGIN
declare @JsonParam sysname = '@json'
declare @JsonSchema nvarchar(max) = codegen.GetOpenJsonSchema (@SchemaName, @TableName, @JsonColumns, @IgnoredColumns, 1);
...
SET @TableSchemaWithAlias = REPLACE(@TableSchemaWithAlias, 'json.IsActive', '1')


declare @TableUpdateColumns nvarchar(max) = '';

select @TableUpdateColumns = @TableUpdateColumns + CHAR(10) + '				' +  codegen.QNAME(ColumnName) + ' = ISNULL(json.' +  codegen.QNAME(ColumnName) + ','+ @TableName+ '.' +codegen.QNAME(ColumnName) + '),'
from codegen.GetTableDefinition(@SchemaName, @TableName, @JsonColumns, @IgnoredColumns)
order by ColumnId
...
declare @Result nvarchar(max) = 
N'CREATE OR ALTER PROCEDURE ' + codegen.QNAME( @ProcSchemaName) + '.' + codegen.QNAME('Entity_' + @TableName + '_Save') +'( @json NVARCHAR(MAX), @context NVARCHAR(MAX))
WITH EXECUTE AS OWNER
AS BEGIN
' +
	codegen.GenerateProcedureHead(@TableName, @JsonParam) 
